...
import { ExternalLink, CheckCircle2, Building2, Search, ArrowUpDown, ArrowUp, ArrowDown, FileText, Phone, Mail, MapPin, Store, Tag } from 'lucide-react';
...
const ResultCard: React.FC<ResultCardProps> = ({ message }) => {
  ...
  const renderMaterialsTable = (data: MaterialItem[]) => (
    <div className="border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 shadow-sm">
      <div className="overflow-x-auto">
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 text-slate-500 dark:text-slate-400 text-[10px] uppercase tracking-widest font-mono">
              ...
              <tr key={idx} className="hover:bg-slate-50 dark:hover:bg-slate-900 transition-colors group">
                <td className="px-6 py-3 align-top">
                  <div className="flex flex-col">
                    <span className="font-medium text-slate-900 dark:text-white">{item.produto}</span>
                    {item.obs && <span className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">{item.obs}</span>}
                  </div>
                </td>
                <td className="px-6 py-3 align-top">
                  <span className="font-mono text-xs text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 border border-slate-200 dark:border-slate-700">{item.marca}</span>
                </td>
                <td className="px-6 py-3 align-top">
                  {item.link ? (
                    <a href={item.link} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 hover:text-blue-600 dark:hover:text-blue-400 hover:underline decoration-1 underline-offset-2 dark:text-slate-300">
                       {item.loja} <ExternalLink size={10} className="opacity-50" />
                    </a>
                  ) : (
                    <span className="text-slate-600 dark:text-slate-300">{item.loja}</span>
                  )}
                </td>
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-800 text-sm">
            {data.map((item, idx) => (
              <tr key={idx} className="hover:bg-slate-50 dark:hover:bg-slate-900 transition-colors">
                <td className="px-6 py-3 font-medium text-slate-900 dark:text-white">{item.nome}</td>
                <td className="px-6 py-3 text-slate-600 dark:text-slate-400 font-mono text-xs">{item.local}</td>
                <td className="px-6 py-3">
                  <a href={`tel:${item.contacto.replace(/[^\d+]/g, '')}`} className="font-mono text-xs text-slate-600 dark:text-slate-300 hover:text-blue-600 dark:hover:text-blue-400 border-b border-dotted border-slate-400 hover:border-blue-600">
                    {item.contacto}
                  </a>
                </td>
                <td className="px-6 py-3">
                  <span className="inline-block px-2 py-0.5 border border-slate-200 dark:border-slate-800 text-[10px] uppercase tracking-wider text-slate-500 dark:text-slate-400">
                     {item.especialidade}
                  </span>
                </td>
              </tr>
  ...
  return (
    <div className="w-full max-w-5xl mx-auto my-12">
      <div className="bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 shadow-sm p-8 relative">
        <div className="absolute top-0 left-0 w-full h-1 bg-slate-900 dark:bg-white"></div>
        <div className="absolute top-4 right-4 text-slate-200 dark:text-slate-800">
          <FileText size={48} strokeWidth={0.5} />
        </div>

        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8 border-b border-slate-100 dark:border-slate-800 pb-6">
          <div>
            <div className="text-[10px] font-mono text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2">Relatório Técnico Gerado por IA</div>
        ...
        <div className="prose prose-slate dark:prose-invert max-w-none prose-p:font-light prose-headings:font-semibold prose-strong:font-medium prose-strong:text-slate-900 dark:prose-strong:text-white">
          {message.content && (
              <div className="mb-8 font-sans leading-relaxed text-slate-700 dark:text-slate-300">
        ...
        {message.sources && message.sources.length > 0 && (
          <div className="mt-12 pt-6 border-t border-slate-100 dark:border-slate-800 flex flex-wrap gap-x-6 gap-y-2">
            <span className="text-[10px] font-mono uppercase tracking-widest text-slate-400 dark:text-slate-500 self-center">Referências:</span>
            {message.sources.map((source, idx) => (
              <a key={idx} href={source.url} target="_blank" rel="noopener noreferrer" className="text-xs text-slate-500 dark:text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 hover:underline decoration-1 underline-offset-2 flex items-center gap-1">
                [{idx + 1}] {source.title}
              </a>
            ))}
          </div>
